...
#' 
#' This function takes frequent itemsets as input and all generates rules of consequent length 1 
#' that have to be considered for rules mining. It uses the Apriori-gen for more efficient
...
#' 
#' This function take the frequent transactions as input and generates rules of consequent length >1.
#' For a rule having consequent length k it will return, if possible, rules with consequent length 
#' k + 1. It uses the Apriori-gen for more efficient generation of rule candidates.
#' @name DetRules_K
...
# Unfortunately, the Matrix package does not provide such a function. We were not able to program
# a function in R that was fast enough. Therefore, we decided to inlcude that function from
# the arules package.
